...
impl MemorySpec {
    fn load(path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for (line_index, raw_line) in text.lines().enumerate() {
            ...
            {
                let (name, kind) = section.split_once(':').ok_or_else(|| {
                    format!("{path}:{line_number}: section must be [name:usize] or [name:isize]")
                })?;
        ...
        for section in &self.sections {
            if let Some(name) = &section.name {
                ...
                if name == "player_offsets" {
                    ...
                    output.push_str(
                        "    pub const BONFIRE_FREE_PEASANT_CAP: isize = POPULATION_ON_BONFIRE;\n",
                    );
                }
...
fn parse_memory_int(value: &str) -> Option<i128> {
    ...
    let (sign, digits) = value
        .strip_prefix('-')
        .map_or((1i128, value.as_str()), |digits| (-1, digits));
